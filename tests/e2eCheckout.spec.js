...
test('E2E test for product checkout ', async ({ page }) => {
    const userEmail = 'noiceataure@gmail.com';
    const userPassword = '@Ata1234';
    const productName = 'adidas original';
    const Cvv = '123';
    const cardName = 'Ataure';
    ...
    await productPage.addToCard(productName);

    //Assert the card details dynamically
    const cartpage = new CartPage(page);
    await cartpage.navigateToMyCart();
    await cartpage.checkMyCartDetails(productName);

    //Fill up the order details
    const checkoutpage = new CheckoutPage(page);
    await checkoutpage.navigateToCheckOutDetails();
    await checkoutpage.inputDetailsToCheckOut(Cvv,cardName,CountryName,userEmail);
